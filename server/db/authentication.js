...
var getTokenById = function (tokenID, cb) {
    db.find('OrderBase', 'AccessToken', {_id: tokenID}, function (err, token) {
        cb(null, token);
    });
};

var getRoleById = function (tokenID, cb) {
    db.find('OrderBase', 'Base', {_id: tokenID}, function (err, token) {
        cb(null, token);
    });
};

var getUserForToken = function (token, cb) {
    db.find('OrderBase', 'User', {_id: token.userID}, function (err, user) {
        cb(null, user);
    });
};
